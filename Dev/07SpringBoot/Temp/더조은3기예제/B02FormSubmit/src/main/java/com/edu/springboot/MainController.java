...
매핑정보를 입력하였다. 하지만 Spring에서는 아래와 같이 @Controller
어노테이션을 부탁하면 자동으로 컨트롤러 클래스로 정의된다. 또한 사용을 
...
public class MainController {
	...
	/*
	컨트롤러에 매핑된 각 메서드에서 String을 반환하면 application.properties
	의 JSP설정에 의해 View의 경로가 조립된다.
	"Prefix경로 + 메서드의반환값 + suffix경로"에 해당하는 JSP파일을 찾아
	포워드하게된다. 즉 이 함수는 index.jsp 파일을 찾아 포워드한다. 
	 */
    ...
    public String sub() {
    	/*
    	만약 views하위에 별도의 폴더가 필요하다면 이와 같이 슬러쉬로 구분하여
    	작성하면된다. sub폴더 하위의 sub.jsp를 찾아 포워드한다. 
    	 */
        return "sub/sub";
    }
    ...
    @RequestParam에 지정된 파라미터명으로 값을 받은 후 우측에 정의한 매개변수에
    저장한다. 
     */
    ...
    사용시 매개변수명은 클래스명의 첫글자만 소문자로 바꾼 이름을 사용해야한다.
    만약 이름을 변경하고 싶다면 @ModelAttribute 어노테이션을 사용한다.
     */
    ...
    public String form3(PersonDTO personDTO) {
    	...
    	또한 Model객체에 저장까지 자동으로 되므로 View로 전달하기 위해
    	별도로 저장할 필요가 없다. 
    	 */
        return "form/submit3";       
    }
