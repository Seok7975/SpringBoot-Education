...
여기서는 별도로 이름을 지정하지 않았으므로 클래스명의 첫글자를 소문자로 
바꾼 student라는 이름으로 빈이 생성된다. 
 */
@Component
public class Student {
	...
	주입받을 수 있다. 이때 @Qualifier 어노테이션이 있으면 빈의 이름까지
	지정해서 주입받는다. 만약 없다면 타입으로만 빈을 찾아 주입받게된다. 
	 */
	@Autowired
	@Qualifier("macBook")
	private Computer notebook;
	
	//생성자, getter, setter, toString() 정의 
	public Student() {}
