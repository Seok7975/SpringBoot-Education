...
public class MainController {
	...
	public String multiUploadProcess(HttpServletRequest req, Model model){
		try {
			...
			//Collection타입으로 반환되므로 확장 for문으로 반복한다. 
			for(Part part : parts) {
				/* 폼값 중 파일인 경우에만 업로드 처리를 위해 continue를 
				걸어준다. 즉 폼값 중 파일이 아닌 경우에는 for문의 처음으로 돌아간다.*/
		        ...
		        String[] phArr = partHeader.split("filename=");
		        String originalFileName = phArr[1].trim().replace("\"", "");
				//파일을 원본파일명으로 서버에 저장한다. 
		        if (!originalFileName.isEmpty()) {				
					part.write(uploadDir+ File.separator +originalFileName);
				}
		        //저장된 파일을 UUID로 생성한 새로운 파일명으로 변경한다. 
				String savedFileName = MyFunctions.renameFile(uploadDir, 
						originalFileName);
				
				/*
				파일명을 Map에 저장한다.
				원본파일명을 Key로 저장된파일명을 Value로 추가한다. 
				 */
