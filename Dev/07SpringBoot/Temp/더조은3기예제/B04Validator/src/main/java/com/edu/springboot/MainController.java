...
public class MainController {
	...
	/*
	Validator 인터페이서를 통한 폼값의 유효성 검증
	Spring에서 사용하는 커맨드객체는 전송된 폼값을 한꺼번에 받아 
	Model인스턴스에 저장해준다. 만약 저장되는 속성명을 변경하고 싶다면
	@ModelAttribute 어노테이션을 사용하면 된다. 아래의 경우 boardDTO가
	...
	public String writeAction1(
		...
		if (result.hasErrors()) {
			//실패한 경우 재입력을 받기위해 쓰기페이지로 포워드한다.
			page = "write";
			/*
			result가 폼값 검증에 대한 모든 결과값을 가지고있다. 따라서
			아래 부분의 출력문을 주석처리 하더라도 여기에서 검증 결과가
			출력된다. 
			 */
	...
	/*
	어노테이션을 통한 검증이므로 폼값 저장을 위한 VO객체에 @Validated
	을 추가한다. 
	 */
	...
	public String writeAction2(
		...
		//검증을 위한 클래스는 별도로 정의할 필요가 없으므로 주석처리 
//		BoardValidator validator = new BoardValidator();
//		validator.validate(boardDTO, result);
